...
#include "../include/RecoilSettings.h"
#include "Logger.h"
#include <algorithm>
#include <cmath>
#include <windows.h>
...
std::pair<int, int> Aiming::ProcessDetections(const std::vector<Detection>& detections, int screen_w, int screen_h) {
...
std::pair<int, int> Aiming::ControlRecoil(const RecoilSettings& recoil_settings)
{
    if (!recoil_settings.enable_recoil_control)
    {
        return { 0, 0 };
    }

    // Check if both left and right mouse buttons are being held down
    if ((GetAsyncKeyState(VK_LBUTTON) & 0x8000) && (GetAsyncKeyState(VK_RBUTTON) & 0x8000))
    {
        // Apply recoil compensation
        // The strength is a general multiplier. The horizontal and vertical values determine the direction and magnitude.
        int recoil_dx = static_cast<int>(recoil_settings.recoil_horizontal * (recoil_settings.recoil_strength / 10.0f));
        int recoil_dy = static_cast<int>(recoil_settings.recoil_vertical * (recoil_settings.recoil_strength / 10.0f));

        return { recoil_dx, recoil_dy };
    }

    return { 0, 0 };
}

void Aiming::ApplyMouseMovement(int dx, int dy)
{
    if ((dx != 0 || dy != 0) && m_mouse)
    {
        m_mouse->move_r(dx, dy);
    }
}
...
void Aiming::MoveMouse(float dx, float dy) {
    if (m_mouse) {
        m_mouse->move_r(static_cast<int>(dx), static_cast<int>(dy));
    }
}
