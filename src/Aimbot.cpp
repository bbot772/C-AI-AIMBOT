    ...
    while (m_running) {
        ...
        if (captured_frame) {
            ...
            if (PreprocessD3D11Texture(captured_frame, m_preprocessed_buffer_gpu, m_capture_width, m_capture_height, m_inference_engine->GetInputWidth(), m_inference_engine->GetInputHeight(), &m_cuda_graphics_resource))
            {
                ...
                // Infer using the preprocessed GPU buffer
                m_inference_engine->Infer(m_preprocessed_buffer_gpu, screen_w, screen_h, new_detections, m_aiming_settings.confidence_threshold, 0.4f);

                // --- NEW: Process detections immediately in the same thread ---
                if (!new_detections.empty()) {
                    m_aiming->ProcessDetections(new_detections, screen_w, screen_h);
                }
