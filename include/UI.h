...
private:
    enum class Tab { Model, Detection, RecoilControl, Performance, Logging, Stealth, Personalization };
    ...
    // For animations
    std::map<Tab, float> m_tab_hover_progress;

    // Sidebar state
    bool m_sidebar_collapsed = false;
    float m_sidebar_anim_width = 150.0f;

    // For context-aware animations
    std::map<ImGuiID, float> m_fade_progress;

    // For hotkey setting is handled by the encapsulated state below
    ...
    bool AnimatedTabButton(const char* icon, const char* label, Tab tab_id, bool collapsed);

    // Animation helpers
    void BeginFade(ImGuiID id, bool show);
    void EndFade();
